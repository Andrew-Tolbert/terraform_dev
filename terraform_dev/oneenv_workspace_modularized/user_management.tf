...
#Running Destroy removes myself from the one env - comment this out on destroy as all users are already in the account 
data "databricks_user" "unity_users" {
  provider  = databricks.mws
...
resource "databricks_group_member" "admin_group_member" {
  ...
  member_id = data.databricks_user.unity_users[each.value].id
}
